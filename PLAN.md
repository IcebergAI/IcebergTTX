...
- JWT authentication (stateless, API-friendly; the browser holds it in an httpOnly cookie only, never JS-readable storage — see #264)
...
### WebSocket
- `GET /ws/exercises/{exercise_id}` — authenticated by the httpOnly `access_token` cookie (no `?token=` param; see #264 / #68)
...
**Auth**: browsers can't set Authorization headers on a WS upgrade, so `exercise_ws` authenticates from the httpOnly `access_token` cookie the browser already sends — there is no `?token=` query-param fallback (a query token is ignored). See the WebSocket auth (#68) subsystem note.
...
- `triggers_communications` delays share `schedule_service`'s keyed registry with inject releases. Pause cancels them, resume subtracts persisted pause spans, and startup reconstructs undelivered triggers; a multi-process deployment still needs a task queue.

---

## Frontend (AlpineJS + Tailwind)

**Auth pattern**: JWT in an `httpOnly` cookie only — the browser uses it for page navigation, `apiFetch` calls, and the WS upgrade; it is never written to JS-readable storage (#264). `get_current_user` still also accepts an `Authorization: Bearer` header (takes precedence) for non-browser API clients, but browser code never sets one.
...
`ws_manager.py` (ConnectionManager), WS router (`/ws/exercises/{id}`, cookie-authenticated), `Inject` model, `inject_service.py` (release + team targeting + WS broadcast), inject router, `exercises/facilitator.html`, `exercises/participant.html`.
...
`Communication` model, `communication_service.py` (visibility filtering by team, durable `triggers_communications` delivery, mark-read), comms router, `communications/inbox.html` + compose form, wired into `inject_service.release_inject` and the shared schedule registry.
...
**Post-commit domain events (#212)**: `app/services/domain_events.py` holds a per-session buffer in `session.info["domain_events"]` (`pending` / `committed`). `record(session, ev)` appends to `pending` and raises if no transaction is open — that is what "you recorded after the commit" looks like. A **synchronous** SQLAlchemy `after_commit` listener on `Session` moves `pending → committed`; `after_soft_rollback` clears `pending`. `AsyncSession.info` *is* the underlying `Session.info` (the same dict), which is what lets the sync listeners see what async code recorded — and both listeners do fire on the savepoint-joined test session (`join_transaction_mode="create_savepoint"`), which the whole design depends on. `await dispatch(session)` drains `committed` **only**, so an uncommitted event cannot be fanned out by construction; the listeners are the only thing that can make an event eligible.

Event catalogue: `ExerciseStateChanged`, `InjectReleased`, `InjectUpdated`, `InjectCommentCreated`, `ResponseSubmitted`, `CommunicationCreated`, `ResponseAssessed`, `InjectSuggested`, `SummaryGenerated`. `ws_projector.py` subscribes one handler per event and is the **only** module that builds a WS frame or names `manager`; `#213` (Redis pub/sub) replaces the fan-out there and no service changes. `InjectReleased` has a **second** subscriber — triggered communications — which is what makes this a bus rather than a broadcast helper; it arms the restart-safe timers `schedule_service` owns (#240), so that durability is unaffected.

Design notes worth keeping. `ExerciseStateChanged` deliberately does **not** carry the `Exercise`: the lifecycle CAS runs `synchronize_session=False`, so the in-session object still holds pre-transition attributes at record time; the handler re-reads it (an identity-map hit). `CommunicationCreated` carries an id, not the row, because the triggered-comms path learns its id from `INSERT … RETURNING` and never loads the object. `ResponseSubmitted` is thin and its handler recomputes `next_injects`/`progression`, both of which are post-commit reads and were computed post-commit before the seam too. Subscriber registration is a lazy import inside `dispatch` — **not** for cycle reasons but for registration *guarantee*: tests call `release_inject` directly without importing `app.main`, and an empty registry would silently drop every frame. `dispatch` swallows and logs handler failures, generalising #129's "a dead socket must not 500 a committed request" from one route to all nine frames. The seam is mutation-tested: breaking the rollback discard, the committed gate, or the terminal `manager` call each fails a specific test — the third matters most, because without the positive tripwire the #129 *negative* rollback test would pass vacuously (its mock simply never consulted).
...
**WebSocket auth (#68)**: browsers can't set the `Authorization` header on a WS upgrade, so `exercise_ws` authenticates from the httpOnly `access_token` **cookie** the browser already sends — keeping the JWT out of the URL (and out of proxy access logs). There is **no** `?token=` query-param fallback — a query token is ignored and a cookieless upgrade is closed `4001` (`test_ws_query_token_is_not_accepted`). The cookie is ambient, so it gets a CSWSH `Origin` check via `origin_allowed()` (`middleware.py`, shared with `CSRFOriginMiddleware`) — closes `4003` on a foreign origin. The user resolves through the shared `resolve_user_from_token()` (`dependencies.py`), so the WS path also enforces `token_valid_after` revocation (#14). The client uses the shared `DT.connectExerciseWs()` helper (`static/js/app.js`): no token in the URL and **no JWT in JS-readable storage at all** (#264; SSO sessions always relied on the cookie). Auth-refused closes never reconnect (retrying can't succeed and each retry emits a server-side audit event) and are handled **per close code** (#264): **4001** — token missing/expired/revoked, including mid-session at the heartbeat, where `ws.py` closes `4001` (not `4003`) so it matches connect-time — clears client state and redirects to `/login` exactly like the `apiFetch` 401 branch, so a live page can't sit silently frozen (clock ticking, no frames) past token expiry; **4003** — origin / exercise-access / membership denied — sets a per-component `wsAccessDenied` flag that the status label renders as "Access denied" instead of redirecting (the user is authenticated but not allowed here). The three page components (`static/js/pages/{exercises,comms}.js`) pass only their `onMessage` handler and a `viewParams` flag. The `exercise_ws` handshake authenticates/authorises against its injected session, then `await session.close()`s it **before** entering the receive loop (the loop is DB-free) — otherwise the dependency-scoped session would hold a pooled connection idle-in-transaction for the whole socket lifetime and ~15 concurrent sockets would exhaust the pool (#35). `broadcast_to_groups` delivers to matching-group connections **plus facilitators and observers** (both have global read-visibility), so observers get live `inject_released`/comment pushes for group-scoped items, matching `is_inject_visible_to_user` (#38).
...
**Runtime email settings (#186)**: `/admin/email` and `/api/email/settings` manage an
`EmailSettings` singleton containing only non-secret SMTP routing and link settings. The
row is lazily seeded from `SMTP_*`; after creation the database is authoritative. A frozen
`MailConfig` cache makes saves immediately visible to synchronous feature gates and delivery,
while an unloaded cache derives from env for lifespan-free transports. `SMTP_PASSWORD`
remains env-only and is read live for delivery. `/api/email/test` accepts no recipient and
can mail only the requesting admin, returning only `ok` or an exception class. Like the
SIEM and proxy caches, this is refreshed across replicas over the config bus (#213).
