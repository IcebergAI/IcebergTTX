...
**Single-replica constraint**: every piece of cross-request state lives in the **process**, so the app must run as one replica (k8s enforces `replicas: 1` + `strategy: Recreate`). **Redis pub/sub alone does not lift this** — it only fixes `ws_manager` fan-out. Also in-memory, each needing its own answer: scheduled inject release (#116) and `triggers_communications`, which fire via `asyncio.create_task(asyncio.sleep(...))` → need a task queue (Celery/ARQ); `rate_limit`, whose per-process counters multiply the effective login/registration/reset limits by the replica count → needs a shared store; and the `SiemConfig`/`ProxyConfig` caches → need cross-replica invalidation. Full table in [deployment.md](website/docs/deployment.md). `schedule_service` keeps an in-memory `_scheduled` registry (`exercise_id → inject_id → task`) so pending timers can be cancelled (release-early), deferred (pause cancels; resume re-arms with the remaining offset), and dropped (complete); `rehydrate_schedules()` in the lifespan re-arms active exercises after a **single-process** restart only.
