...
**Scenario branching**: "Pull not push" — when a participant responds, the service resolves which inject IDs are valid next steps, but the facilitator manually reviews and releases the chosen branch. This keeps human control in the loop. **Reading the branch targets off a response is a trap**: `next_inject_ids` holds *scenario-node* ids (`'containment'`) and will never match an `Inject.id` — use `next_injects[].id`. And only `GET /responses` populates `next_injects`; the `POST` reply omits it, so code that reads it there silently resolves to nothing. A response also resolves the inject for the **whole team** and advances that team's cursor, so another branch can no longer be released to them (`409 Inject is not the current branch for its group`).
