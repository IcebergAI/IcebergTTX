...
async def _inject_node(
    session: AsyncSession, inject: Inject, definition: ScenarioDefinition | None = None
):
    if not inject.scenario_node_id:
        return None
    from app.services.scenario_service import definition_for_exercise, get_inject_node

    definition = definition or await definition_for_exercise(session, inject.exercise_id)
    if not definition:
        return None
